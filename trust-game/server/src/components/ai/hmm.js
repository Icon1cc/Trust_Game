...
/**
 * Update the AI state based on Profit & Loss (PnL).
 * @param {string} state - Current AI state.
 * @param {number} PnL - Profit or Loss value.
 * @returns {string} - Updated state.
 */
export function updateState(state, PnL) {
    const states = ["unhappy", "neutral", "happy"];
    const modifiers = {
        unhappy: [Math.exp(0.0), Math.exp(-3.366027 + 0.40910797 * PnL), Math.exp(-3.572619 - 0.08137274 * PnL)],
        neutral: [Math.exp(0.0), Math.exp(3.3142637 + 0.3763408 * PnL), Math.exp(0.9169736 + 0.4502838 * PnL)],
        happy: [Math.exp(0.0), Math.exp(0.7134085 + 0.02101626 * PnL), Math.exp(2.2215478 + 0.16162964 * PnL)],
    };
 ...
 */
export function getInvestment(state) {
